...
class RoleCardRenderer:
    ...
    def render_role_card(self, role_detail) -> bytes:
        """渲染角色练度卡片"""
        ...
        # 绘制顶部信息栏（账号等级、世界等级等）
        self._draw_header_section(img, role_detail)
        
        # 绘制角色信息区域（左侧立绘 + 右侧属性）
        self._draw_role_section(img, role_detail)
        
        # 绘制属性面板
        self._draw_property_section(img, role_detail)
    ...
    def _draw_header_section(self, img: Image.Image, role_detail):
        """绘制顶部信息栏"""
    ...
    def _draw_property_section(self, img: Image.Image, role_detail):
        """绘制属性面板（右侧）"""
        draw = ImageDraw.Draw(img)
        ...
        # 获取属性数据
        props = self._get_role_properties(role_detail)
    ...
    def _get_role_properties(self, role_detail) -> Dict[str, str]:
        """获取角色属性数据"""
        props = {
            "生命": "16743",
            "攻击": "2382",
        ...
        # TODO: 从role_detail中提取真实属性数据
        # 目前API返回的数据中可能没有完整属性，需要后续完善
...
def render_role_card(role_detail) -> bytes:
    """渲染角色练度卡片（便捷函数）"""
    renderer = get_renderer()
    return renderer.render_role_card(role_detail)
